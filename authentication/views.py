...
from .serializers import APIKeySerializer, ChangePasswordSerializer, LoginSerializer, UserCreateSerializer, UserSerializer
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        login_attempt = LoginAttempt.objects.create(
            email=email or "", ip_address=ip_address, user_agent=user_agent, success=False
        )
...
def register_view(request):
    ...
    if serializer.is_valid():
        try:
            ...
            return Response({"error": "Erro ao criar usuário", "details": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
def logout_view(request):
    ...
    try:
        ...
        return Response({"error": "Erro ao realizar logout", "details": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class APIKeyViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(user=self.request.user, key=hashlib.sha256(key.encode()).hexdigest())
    ...
    def revoke(self, request, pk=None):
        ...
        return Response({"message": "API Key revogada com sucesso"}, status=status.HTTP_200_OK)
...
def security_stats(request):
    ...
    stats = {
        ...
        "failed_logins_24h": LoginAttempt.objects.filter(created_at__gte=last_24h, success=False).count(),
        "active_users": User.objects.filter(is_active=True, last_login__gte=last_7d).count(),
