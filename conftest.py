...
def profissional_psicologo(db, endereco_sample):
    ...
    endereco_psi = Endereco.objects.create(
        logradouro="Avenida Paulista", numero="1000", bairro="Bela Vista", cidade="São Paulo", estado="SP", cep="98765432"
    )
...
def jwt_token_paciente(paciente_user):
    ...
    return {"refresh": str(refresh), "access": str(refresh.access_token), "user": paciente_user}
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "models: marca testes de modelos")
    config.addinivalue_line("markers", "serializers: marca testes de serializers")
    config.addinivalue_line("markers", "views: marca testes de views")
    config.addinivalue_line("markers", "validators: marca testes de validadores")
