...
from lacrei_saude.validators import (
    validate_phone, validate_name, sanitize_email, 
    validate_money_amount, validate_observation
)
...
class ConsultaSerializer(BaseModelSerializer, TimestampsMixin, ValidationMixin):
    ...
    class Meta:
        model = Consulta
        fields = [
            'id', 'profissional', 'profissional_info',
            'data_hora', 'data_hora_formatada', 'duracao_estimada',
            'data_hora_fim', 'data_hora_fim_formatada', 'duracao_real_minutos',
            'tipo_consulta', 'tipo_consulta_display', 'status', 'status_display',
            'nome_paciente', 'telefone_paciente', 'email_paciente',
            'motivo_consulta', 'observacoes', 'observacoes_internas',
            'valor_consulta', 'forma_pagamento', 'forma_pagamento_display', 'pago',
            'motivo_cancelamento', 'cancelado_por', 'data_cancelamento',
            'consulta_origem', 'tempo_restante_minutos', 'pode_cancelar', 'pode_remarcar',
    ...
    def validate_duracao_estimada(self, value):
        ...
        if value > 480:  # 8 horas
            raise serializers.ValidationError(
                "Duração estimada não pode exceder 8 horas (480 minutos)"
            )
        return value
...
class ConsultaListSerializer(BaseModelSerializer):
    ...
    class Meta:
        model = Consulta
        fields = [
            'id', 'profissional_nome', 'profissional_profissao',
            'data_hora', 'data_hora_formatada', 'tipo_consulta',
            'status', 'status_display', 'nome_paciente',
            'valor_consulta', 'pago'
        ]
...
class ConsultaCreateSerializer(ConsultaSerializer):
    ...
    class Meta(ConsultaSerializer.Meta):
        fields = [
            'profissional', 'data_hora', 'duracao_estimada', 'tipo_consulta',
            'nome_paciente', 'telefone_paciente', 'email_paciente',
            'motivo_consulta', 'observacoes', 'valor_consulta'
        ]
        extra_kwargs = {
...
class ConsultaUpdateSerializer(ConsultaSerializer):
    ...
    class Meta(ConsultaSerializer.Meta):
        fields = [
            'data_hora', 'duracao_estimada', 'telefone_paciente',
            'email_paciente', 'motivo_consulta', 'observacoes',
            'valor_consulta', 'forma_pagamento'
        ]
    ...
    def validate(self, data):
        ...
        if self.instance and self.instance.status in ['CONCLUIDA', 'CANCELADA']:
            raise serializers.ValidationError(
                "Não é possível alterar consulta finalizada"
            )
...
class ConsultaActionSerializer(serializers.Serializer):
    ...
    cancelado_por = serializers.ChoiceField(
        choices=[('PACIENTE', 'Paciente'), ('PROFISSIONAL', 'Profissional'), ('SISTEMA', 'Sistema')],
        required=False
    )
    ...
    def validate(self, data):
        ...
        if action == 'cancelar':
            if not data.get('motivo'):
                raise serializers.ValidationError({
                    'motivo': 'Motivo é obrigatório para cancelamento'
                })
        
        if action == 'remarcar':
            if not data.get('nova_data_hora'):
                raise serializers.ValidationError({
                    'nova_data_hora': 'Nova data e hora são obrigatórias para remarcação'
                })
            
            # Validar se nova data é no futuro
            if data['nova_data_hora'] <= timezone.now():
                raise serializers.ValidationError({
                    'nova_data_hora': 'Nova data deve ser no futuro'
                })
...
class ConsultaPacienteSerializer(BaseModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'profissional_nome', 'profissional_profissao',
            'endereco_atendimento', 'data_hora', 'data_hora_formatada',
            'duracao_estimada', 'tipo_consulta', 'status', 'status_display',
            'motivo_consulta', 'observacoes', 'valor_consulta', 'pago'
        ]
