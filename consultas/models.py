...
class Consulta(BaseModelWithManager):
    ...
    profissional = models.ForeignKey(
        ...
        help_text="Profissional responsável pela consulta"
    )
    
    # Data e hora
    data_hora = models.DateTimeField(
        help_text="Data e hora agendada para a consulta"
    )
    ...
    data_hora_fim = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Data e hora de término (calculada ou real)"
    )
    ...
    tipo_consulta = models.CharField(
        ...
        help_text="Tipo da consulta"
    )
    status = models.CharField(
        max_length=20,
        choices=STATUS_CHOICES,
        default='AGENDADA',
        help_text="Status atual da consulta"
    )
    ...
    telefone_paciente = models.CharField(
        max_length=20,
        help_text="Telefone do paciente"
    )
    email_paciente = models.EmailField(
        blank=True,
        help_text="Email do paciente (opcional)"
    )
    ...
    motivo_consulta = models.TextField(
        max_length=500,
        blank=True,
        help_text="Motivo principal da consulta"
    )
    ...
    observacoes_internas = models.TextField(
        blank=True,
        max_length=1000,
        help_text="Observações internas (não visíveis ao paciente)"
    )
    ...
    valor_consulta = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Valor cobrado pela consulta"
    )
    ...
    motivo_cancelamento = models.TextField(
        blank=True,
        max_length=500,
        help_text="Motivo do cancelamento (se aplicável)"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(duracao_estimada__gt=0),
                name='duracao_positiva'
            ),
            models.CheckConstraint(
                condition=models.Q(valor_consulta__gte=0) | models.Q(valor_consulta__isnull=True),
    ...
    def cancelar(self, motivo, cancelado_por='SISTEMA'):
        ...
        self.save(update_fields=[
            ...
            'cancelado_por', 'data_cancelamento', 'updated_at'
        ])
