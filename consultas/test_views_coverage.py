    ...
    def test_custom_actions(self):
        ...
        for action in custom_actions:
            ...
            response = self.client.post(action_url, {
                'motivo': f'Teste de {action}',
                'data_horario': (datetime.now() + timedelta(days=15)).isoformat()
            }, format='json')
    ...
    def test_search_functionality(self):
        ...
        if response.status_code == 200:
            ...
            if 'results' in data and len(data['results']) > 0:
                found_match = any('joão' in result.get('nome_paciente', '').lower() 
...
class ConsultaFiltersTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        prof_user = User.objects.create_user(
            username='prof_filter',
            email='prof_filter@test.com',
            password='TestPassword123!',
            user_type='profissional'
        )
