...
class TestConsultaAPIAuthenticated(TestCase):
    ...
    def test_create_consulta_admin(self):
        ...
        self.assertTrue(Consulta.objects.filter(nome_paciente="Maria Silva").exists())
    ...
    def test_update_consulta_admin(self):
        ...
        update_data = {"motivo_consulta": "Consulta de retorno", "observacoes": "Paciente retornando para reavaliação"}
...
class TestConsultaAPIValidation(TestCase):
    ...
    def test_update_consulta_finalizada(self):
        ...
        update_data = {"observacoes": "Tentativa de alteração"}
...
class TestConsultaAPIActions(TestCase):
    ...
    def test_cancelar_consulta(self):
        ...
        if hasattr(self, "consulta_cancelar_url"):
            ...
            response = self.client.post(url, {"motivo_cancelamento": "Paciente não pode comparecer"})
        else:
            # Atualizar status para cancelada
            response = self.client.patch(
                url, {"status": "CANCELADA", "motivo_cancelamento": "Paciente não pode comparecer"}, format="json"
            )
...
class TestConsultaAPIEdgeCases(TestCase):
    ...
    def test_create_consulta_horario_noturno(self):
        ...
        """
        # Consulta às 22h
        data_noturna = timezone.now().replace(hour=22, minute=0, second=0, microsecond=0) + timedelta(days=1)
    ...
    def test_consulta_valor_automatico(self):
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertEqual(Decimal(response.data["valor_consulta"]), self.profissional.valor_consulta)
