...
class TestUserRoleAuthorization(TestCase):
    ...
    def test_admin_full_access_profissionais(self):
        ...
        self.assertIn(response.status_code, [
            status.HTTP_201_CREATED,  # Sucesso
    ...
    def test_common_user_no_access(self):
        ...
        self.assertIn(response.status_code, [
            status.HTTP_403_FORBIDDEN,  # Negado
            status.HTTP_200_OK  # Permitido (depende das regras)
        ])
    ...
    def test_paciente_limited_access_consultas(self):
        ...
        response = self.client.get(url)
        self.assertIn(response.status_code, [
            status.HTTP_200_OK,  # Permitido (suas consultas)
            status.HTTP_403_FORBIDDEN  # Negado
        ])
    ...
    def test_superuser_status_authorization(self):
        ...
        endpoints = [
            'profissionais:profissional-list',
            'consultas:consulta-list'
        ]
...
class TestPermissionEdgeCases(TestCase):
    ...
    def test_multiple_concurrent_sessions(self):
        ...
        for token in [token1, token2]:
            ...
            self.assertIn(response.status_code, [
                status.HTTP_200_OK,
                status.HTTP_403_FORBIDDEN  # Dependendo das permissões
            ])
    ...
    def test_case_sensitivity_permissions(self):
        ...
        user_lower = User.objects.create_user(
            username='lower@test.com',
            email='lower@test.com',
            password='lower123',
            tipo_usuario='admin'  # Minúsculo
        )
