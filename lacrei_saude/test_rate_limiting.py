...
class RateLimitingTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", email="test@test.com", password="TestPassword123!", user_type="paciente"
        )
        cache.clear()
    ...
    def test_password_reset_rate_limiting(self):
        ...
        for i in range(4):  # Assumindo limite de 3 por hora
            response = self.client.post(reset_url, {"email": "test@test.com"})
...
class BruteForceProtectionTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="brutetest", email="brute@test.com", password="TestPassword123!", user_type="paciente"
        )
        cache.clear()
    ...
    def test_brute_force_login_protection(self):
        ...
        password_attempts = ["password123", "admin", "123456", "password", "letmein", "wrongpassword"]

        blocked = False
        for password in password_attempts:
            response = self.client.post(login_url, {"username": "brutetest", "password": password})
    ...
    def test_ip_based_rate_limiting(self):
        ...
        for i in range(10):
            response = self.client.post(
                login_url, {"username": f"user{i}", "password": "wrongpassword"}, HTTP_X_FORWARDED_FOR="192.168.1.100"
            )
            responses.append(response.status_code)
    ...
    def test_account_lockout_protection(self):
        ...
        for i in range(6):
            response = self.client.post(login_url, {"username": "brutetest", "password": "wrongpassword"})
        ...
        response = self.client.post(login_url, {"username": "brutetest", "password": "TestPassword123!"})
...
class RateLimitHeadersTestCase(APITestCase):
    ...
    def test_rate_limit_headers_present(self):
        ...
        headers_found = any(header in response.headers or header.lower() in response.headers for header in expected_headers)
...
class RateLimitBypassTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="bypasstest", email="bypass@test.com", password="TestPassword123!", user_type="paciente"
        )
        cache.clear()
    ...
    def test_user_agent_rotation_bypass(self):
        ...
        for i, user_agent in enumerate(user_agents * 3):  # 9 tentativas
            response = self.client.post(
                login_url, {"username": "bypasstest", "password": "wrongpassword"}, HTTP_USER_AGENT=user_agent
            )
    ...
    def test_header_manipulation_bypass(self):
        ...
        for i, headers in enumerate(headers_variants * 3):  # 12 tentativas
            response = self.client.post(login_url, {"username": "bypasstest", "password": "wrongpassword"}, **headers)
    ...
    def test_distributed_attack_simulation(self):
        ...
        for i in range(10):
            ...
            for j in range(3):
                response = self.client.post(
                    login_url, {"username": "bypasstest", "password": "wrongpassword"}, HTTP_X_FORWARDED_FOR=ip
                )
...
class RateLimitingIntegrationTestCase(APITestCase):
    ...
    def test_rate_limiting_across_endpoints(self):
        ...
        endpoints = ["/api/v1/profissionais/", "/api/v1/consultas/", "/api/auth/status/", "/api/security/check/"]
    ...
    def test_rate_limiting_recovery(self):
        ...
        # Força rate limiting
        for i in range(10):
            self.client.post(login_url, {"username": "integration", "password": "wrongpassword"})

        # Espera um pouco (simula reset do rate limit)
        time.sleep(1)

        # Tenta novamente - deveria eventualmente recuperar
        recovery_response = self.client.post(login_url, {"username": "integration", "password": "TestPassword123!"})
