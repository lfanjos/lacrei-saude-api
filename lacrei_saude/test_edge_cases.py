...
class TestTimezoneEdgeCases(TestCase):
    ...
    def setUp(self):
        ...
        self.endereco = Endereco.objects.create(
            logradouro="Rua Timezone", numero="100", bairro="Timezone", cidade="São Paulo", estado="SP", cep="12345678"
        )
    ...
    def test_consulta_mudanca_horario_verao(self):
        ...
        data_mudanca = timezone.now().replace(month=10, day=15, hour=2, minute=30) + timedelta(days=30)  # 30 dias no futuro
    ...
    def test_consulta_meia_noite(self):
        ...
        meia_noite = timezone.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
    ...
    def test_consulta_31_dezembro(self):
        ...
        fim_ano = timezone.now().replace(month=12, day=31, hour=23, minute=59)
    ...
    def test_consulta_ano_bissexto(self):
        ...
        data_bissexto = timezone.now().replace(year=ano_bissexto, month=2, day=29, hour=14, minute=0)
...
class TestBoundaryValues(TestCase):
    ...
    def test_valor_consulta_extremos(self):
        ...
        endereco = Endereco.objects.create(
            logradouro="Rua Valor", numero="100", bairro="Valor", cidade="São Paulo", estado="SP", cep="12345678"
        )
        ...
        for valor in valores_teste:
            ...
            if float(valor) > 0:
                self.assertIn(
                    response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]  # Se houver limite de valor
                )
    ...
    def test_duracao_consulta_extremos(self):
        ...
        # Criar profissional
        endereco = Endereco.objects.create(
            logradouro="Rua Duração", numero="100", bairro="Duração", cidade="São Paulo", estado="SP", cep="12345678"
        )
...
class TestConcurrencyEdgeCases(TestCase):
    ...
    def setUp(self):
        ...
        # Criar profissional
        self.endereco = Endereco.objects.create(
            logradouro="Rua Concorrência", numero="100", bairro="Concorrência", cidade="São Paulo", estado="SP", cep="12345678"
        )
    ...
    def test_atualizacao_simultanea_mesmo_objeto(self):
        ...
        self.assertIn(consulta.nome_paciente, ["Paciente Atualizado 1", "Paciente Atualizado 2"])
...
class TestSpecialCharactersEdgeCases(TestCase):
    ...
    def test_nomes_caracteres_especiais(self):
        ...
        for i, nome in enumerate(nomes_especiais):
            ...
            self.assertIn(
                response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]  # Se houver validação restritiva
            )
    ...
    def test_enderecos_caracteres_especiais(self):
        ...
        enderecos_especiais = [
            {"logradouro": "Rua São João", "bairro": "Coração de Jesus", "cidade": "São Luís"},
            {"logradouro": "Avenida Brasília", "bairro": "Água Branca", "cidade": "Ribeirão Preto"},
            {"logradouro": "Praça da Sé", "bairro": "Centro Histórico", "cidade": "Salvador"},
        ]
