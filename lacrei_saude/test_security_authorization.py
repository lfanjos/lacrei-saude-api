...
class TestHorizontalAuthorization(TestCase):
    ...
    def test_paciente_nao_acessa_consulta_outro_paciente(self):
        ...
        url = reverse('consultas:consulta-detail', 
                     kwargs={'pk': self.consulta_paciente2.pk})
    ...
    def test_paciente_nao_modifica_consulta_outro_paciente(self):
        ...
        update_data = {
            'observacoes': 'Tentativa de modificação maliciosa'
        }
        ...
        self.assertNotEqual(self.consulta_paciente2.observacoes, 
    ...
    def test_paciente_nao_deleta_consulta_outro_paciente(self):
        ...
        self.assertTrue(
            Consulta.objects.filter(pk=self.consulta_paciente2.pk).exists()
        )
...
class TestAPIKeyAuthorization(TestCase):
    ...
    def setUp(self):
        ...
        self.api_key = APIKey.objects.create(
            name='Test API Key',
            user=self.user,
            permissions={'profissionais': ['read', 'write']}
        )
        ...
        self.inactive_api_key = APIKey.objects.create(
            ...
            user=self.user,
            is_active=False,
            permissions={'profissionais': ['read']}
        )
    ...
    def test_api_key_valida(self):
        ...
        self.assertIn(response.status_code, [
            status.HTTP_200_OK,
            status.HTTP_401_UNAUTHORIZED  # Se API Key auth não implementada
        ])
    ...
    def test_api_key_permissions(self):
        ...
        limited_key = APIKey.objects.create(
            ...
            user=self.user,
            permissions={'consultas': ['read']}  # Apenas leitura de consultas
        )
        ...
        self.assertIn(response.status_code, [
            status.HTTP_200_OK,  # Se não implementado
