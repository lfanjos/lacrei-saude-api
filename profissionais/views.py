...
from .serializers import (
    ProfissionalSerializer, ProfissionalListSerializer,
    ProfissionalCreateSerializer, ProfissionalDetailSerializer
)
from .filters import ProfissionalFilter
from lacrei_saude.pagination import StandardResultsSetPagination
from authentication.permissions import (
    IsOwnerOrAdmin, IsProfissionalOrAdmin, ReadOnlyOrOwner
)
...
class ProfissionalViewSet(viewsets.ModelViewSet):
    ...
    # Campos para busca
    search_fields = [
        'nome_social', 'nome_registro', 'especialidade', 
        'endereco__cidade', 'endereco__bairro'
    ]
    
    # Campos para ordenação
    ordering_fields = [
        'nome_social', 'profissao', 'created_at', 'valor_consulta',
        'especialidade', 'endereco__cidade', 'endereco__estado'
    ]
        ...
        if valor_max:
            try:
                ...
                queryset = queryset.filter(
                    Q(valor_consulta__lte=valor_max) | Q(valor_consulta__isnull=True)
                )
            except ValueError:
                pass
    ...
    def estatisticas(self, request):
        ...
        profissoes_stats = queryset.values('profissao').annotate(
            total=Count('id'),
            valor_medio=Avg('valor_consulta')
        )
