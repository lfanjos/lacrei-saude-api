...
class EnderecoListSerializer(EnderecoSerializer):
    ...
    class Meta(EnderecoSerializer.Meta):
        fields = ["id", "logradouro", "numero", "bairro", "cidade", "estado", "cep", "endereco_completo"]
...
class ProfissionalSerializer(BaseModelSerializer, TimestampsMixin, ValidationMixin):
    ...
    def validate_email(self, value):
        ...
        if value:
            ...
            if queryset.exists():
                raise serializers.ValidationError("Já existe um profissional ativo com este email")
...
class ProfissionalDetailSerializer(ProfissionalSerializer):
    ...
    class Meta(ProfissionalSerializer.Meta):
        fields = ProfissionalSerializer.Meta.fields + ["total_consultas", "consultas_mes_atual", "media_avaliacao"]
    ...
    def get_consultas_mes_atual(self, obj):
        ...
        return obj.consultas.filter(is_active=True, data_hora__date__gte=primeiro_dia_mes, data_hora__date__lte=hoje).count()
